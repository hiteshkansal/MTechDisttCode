...
public class SummarizeProduct 
{
	...
	while(cluster[i]!=null)
	{
		...
		total = count;
            ...
            System.out.println("");
            System.out.println("Feature having positive summary: "+cluster[i].feature);
            int lineNo = 1;
            for(Iterator itr = cluster[i].sNo.iterator();itr.hasNext();)
            {
		int n = (Integer) itr.next();
                String ln = productSentence.get(n);
                if(ln==null)
                {
                    n--;
                }
        ...
        for(int i = 0; i < i1 ; i++)
	{
            int lineNo = 1;
            System.out.println("");
            System.out.println("Feature having negative summary: "+cluster[i].feature);
            for(Iterator itr = cluster[i].sNo.iterator();itr.hasNext();)
            {
		int n = (Integer) itr.next();
                String ln = productSentence.get(n);
